...
def construct_nn(ann_input, ann_output):
    ...
    init_in_1 = np.random.randn(in_shape, n_hidden).astype(theano.config.floatX)
    init_1_2 = np.random.randn(n_hidden, n_hidden).astype(theano.config.floatX)
    init_2_out = np.random.randn(n_hidden).astype(theano.config.floatX)

    with pm.Model() as neural_network:
        weights_in_1 = pm.Normal('w_in_1', 0, sd=1,
                                 shape=(in_shape, n_hidden), testval=init_in_1)
        ...
        pm.Normal('out', act_out, observed=ann_output,
                  total_size=out_len  # Important for mini-batches
                  )
...
def main(ml_model, *args):
    ...
    for i, k in enumerate(dict_X_train_ss.keys()):
        ...
        with neural_network:
            ...
            plt.plot(-inference.hist, label='ADVI', color='r', alpha=0.3)
            plt.xlabel('Iteration'); plt.ylabel('ELBO'); plt.legend()

        ann_input.set_value(Xt)
        ann_output.set_value(yt)
        with neural_network:
            ppc = pm.sample_ppc(trace, samples=10000, progressbar=True)
            #Mean of the pred distr.
            y_pred = ppc['out'].mean(axis=0)
            print('Accuracy: {}'.format((abs(yt - y_pred) < 0.01).mean() * 100))
    ...
    #print ("Model parameters: {}".format(neural_network))
